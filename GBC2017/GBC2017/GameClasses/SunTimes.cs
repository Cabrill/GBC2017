...
#if DEBUG
                if ((Math.Sign(zone) == Math.Sign(lon)) && (zone != 0))
                {
                    Debug.Print("WARNING: time zone and longitude are incompatible!");
                    //Throwing an exception here, because I want to detect/correct this if necessary
                    throw new InvalidDataException("time zone and longitude are incompatible");
                    return false;
                }
#endif
                lon = lon / 360;
